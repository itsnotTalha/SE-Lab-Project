import { CalendarDays, Eye, Fingerprint, Search } from 'lucide-react';
...
export default function AssetSelector({ assets, selected, onSelect, onPreview }) {
	...
	return <div className="verification-asset-selector"><label className="search-field"><Search size={15}/><span className="sr-only">Search registered assets</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search your assets"/></label><div className="verification-asset-options" role="listbox" aria-label="Registered assets">{filtered.map((asset) => <button type="button" role="option" aria-selected="false" className="verification-asset-option" key={asset.id} onClick={() => onSelect(asset)}><span><AssetThumbnail asset={asset}/></span><div><strong>{asset.title}</strong><small>Asset #{asset.id} · {dimensions(asset)}</small><small>{asset.createdAt ? `Registered ${new Date(asset.createdAt).toLocaleDateString()}` : 'Registration date unavailable'}</small></div>{asset.hasHash ? <Fingerprint size={15}/> : null}</button>)}</div>{filtered.length === 0 ? <p className="verification-selector-empty">{assets.length ? 'No assets match your search.' : 'Upload an asset before running verification.'}</p> : null}</div>;
}
