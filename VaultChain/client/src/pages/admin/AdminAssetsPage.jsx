...
export default function AdminAssetsPage() {
	...
	return <>{header}{actionError ? <div className="error-banner">{actionError}</div> : null}<div className="admin-stats-grid"><StatCard label="Assets created" value={formatInteger(summary.total)} detail="Within selected period" icon={Images}/><StatCard label="Verified assets" value={formatInteger(summary.verified)} detail={summary.total ? `${(summary.verified/summary.total*100).toFixed(1)}% coverage` : 'No assets'} icon={FileCheck2} tone="green"/><StatCard label="Flagged assets" value={formatInteger(summary.suspicious)} detail="Review or suspicious status" icon={AlertTriangle} tone="red"/><StatCard label="Pending verification" value={formatInteger(summary.pending)} detail="No verification report" icon={Clock3} tone="amber"/></div><section className="admin-card"><div className="admin-table-toolbar"><label><Search size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search asset, owner, or reference"/></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option>All assets</option><option>Verified</option><option>Pending</option><option>Review</option><option>Suspicious</option><option>Suspended</option></select></div>{rows.length ? <AssetTable rows={rows} canManage onStatusChange={updateStatus}/> : <AdminEmpty title="No matching assets" description="Try another filter or date range."/>}</section></>;
}
