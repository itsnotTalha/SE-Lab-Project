...
export default function AdminUsersPage() {
	...
	return <>{header}{actionError ? <div className="error-banner">{actionError}</div> : null}<div className="admin-stats-grid"><StatCard label="Total users" value={formatInteger(summary.total)} detail={`${formatInteger(summary.new_users)} joined in this period`} icon={UsersRound}/><StatCard label="Active users" value={formatInteger(summary.active)} detail={summary.total ? `${((summary.active / summary.total) * 100).toFixed(1)}% of accounts` : 'No accounts'} icon={UserCheck} tone="green" delay={.05}/><StatCard label="Admin accounts" value={formatInteger(summary.admins)} detail="Privileged roles" icon={ShieldCheck} tone="blue" delay={.1}/><StatCard label="Under review" value={formatInteger(summary.review)} detail="Requires moderation" icon={UsersRound} tone="amber" delay={.15}/></div><section className="admin-card"><div className="admin-table-toolbar"><label><Search size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by user, email, or ID"/></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option>All users</option><option>Active</option><option>Review</option><option>Suspended</option></select></div>{rows.length ? <UserTable rows={rows} canManage onRoleChange={(id, role) => update(id, { role })} onStatusChange={(id, value) => update(id, { status: value.toLowerCase() })}/> : <AdminEmpty title="No matching users" description="Try another filter or search term."/>}</section></>;
}
