import {
	...
	ShieldCheck, Sparkles, UploadCloud, X, Zap, Sun, Moon,
} from 'lucide-react';
			...
			<div className="landing-container landing-nav__inner">
				...
				<nav className={`landing-nav__links ${open ? 'is-open' : ''}`} aria-label="Landing page navigation">
					...
					<div className="landing-nav__mobile-actions">
						<button type="button" className="landing-button landing-button--secondary theme-toggle" onClick={() => { toggleTheme(); close(); }} aria-label="Toggle theme" style={{ padding: '0 10px' }}>{theme === 'dark' ? <Sun size={17}/> : <Moon size={17}/>}</button>
						{isAuthenticated ? <Link to="/dashboard" className="landing-button landing-button--primary" onClick={close}>Dashboard</Link> : <><Link to="/login" className="landing-button landing-button--secondary" onClick={close}>Sign in</Link><Link to="/register" className="landing-button landing-button--primary" onClick={close}>Get started</Link></>}
					</div>
				</nav>
				<div className="landing-nav__actions">
					<button type="button" className="landing-button landing-button--ghost theme-toggle" onClick={toggleTheme} aria-label="Toggle theme" style={{ padding: '0 10px' }}>{theme === 'dark' ? <Sun size={18}/> : <Moon size={18}/>}</button>
					{isAuthenticated ? <Link to="/dashboard" className="landing-button landing-button--primary">Dashboard <ArrowRight size={14} /></Link> : <><Link to="/login" className="landing-button landing-button--ghost">Sign in</Link><Link to="/register" className="landing-button landing-button--primary">Get started <ArrowRight size={14} /></Link></>}
				</div>
