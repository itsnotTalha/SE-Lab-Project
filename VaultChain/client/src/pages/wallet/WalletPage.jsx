	...
	return (
		<>
			...
			<div className="dashboard-grid">
				<SectionCard title="Transaction history" description="Entries returned by the authenticated wallet API.">{transactions.length === 0 && !loading ? <EmptyState icon={WalletCards} title="No transactions yet" description="New deposits and withdrawals will appear here."/> : <div className="transaction-list">{transactions.map((item)=>{const credit=CREDIT_TYPES.has(item.type);return <div className="transaction-item" key={item.id}><span className={credit?'is-credit':'is-debit'}>{credit?<ArrowDownLeft size={16}/>:<ArrowUpRight size={16}/>}</span><div><strong>{item.description || item.type}</strong><small>{item.type} · {new Date(item.createdAt).toLocaleString()}{item.referenceId ? ` · Ref ${item.referenceId}` : ''}</small></div><b className={credit?'is-credit':'is-debit'}>{credit?'+':'−'}{Number(item.amount).toLocaleString()} credits</b></div>})}</div>}</SectionCard>
