import { FolderLock, FolderPlus, Images, KeyRound, Layers3, ShieldCheck } from 'lucide-react';
...
export default function VaultPage() {
	...
	const protectedVaults = vaults.filter((vault) => vault.passwordProtected).length;
	const securityScore = vaults.length ? Math.round(70 + (protectedVaults / vaults.length) * 25) : 70;
	return <><PageHeader eyebrow="Private collections" title="Vaults" description="Organize registered assets behind additional access controls and time-limited unlock sessions." action={<Button icon={FolderPlus} onClick={() => setCreateOpen(true)}>Create Vault</Button>}/>{error ? <div className="error-banner">{error}</div> : null}{loading ? <LoadingState label="Loading your Vaults"/> : <>{vaults.length ? <><div className="vault-security-hero"><div><span><ShieldCheck size={21}/></span><div><small>Workspace security level</small><h2>{securityScore}% protected</h2><p>Password controls and authenticated asset access are active.</p></div></div><div className="vault-security-meter"><span><i style={{ width: `${securityScore}%` }}/></span><small>{protectedVaults} of {vaults.length} vaults password protected</small></div><div className="vault-security-fact"><KeyRound size={17}/><span><strong>Auto-lock ready</strong><small>Unlocked sessions expire automatically</small></span></div></div><div className="vault-stats">{statCards.map((stat) => <StatCard key={stat.label} {...stat}/>)}</div><div className="vault-grid">{vaults.map((vault) => <VaultCard key={vault.reference} vault={vault}/>)}</div></> : <EmptyState icon={FolderLock} title="No Vaults yet" description="Create private collections to organize your registered VaultChain assets without making additional file copies." action={<Button icon={FolderPlus} onClick={() => setCreateOpen(true)}>Create your first Vault</Button>}/>}</>}<VaultFormModal open={createOpen} onClose={() => setCreateOpen(false)} onSubmit={create}/></>;
}
