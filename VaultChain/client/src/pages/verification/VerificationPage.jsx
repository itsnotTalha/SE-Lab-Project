...
export default function VerificationPage() {
	...
	async function findMatches() {
		...
		try {
			...
			const nextReport = await verificationService.create(comparisonFile);
			setReport({ ...nextReport, clientProcessingMs: performance.now() - startedAt });
	...
	return <><PageHeader eyebrow="Evidence workspace" title="Verification Center" description="Compare an image against registered fingerprints and inspect every step behind the result."/>{error?<div className="error-banner verification-error" role="alert"><AlertCircle size={16}/>{error}<button type="button" onClick={()=>setError('')} aria-label="Dismiss error">×</button></div>:null}{report?<VerificationReport report={report} history={history} sourceUrl={comparisonPreviewUrl} onVerifyAnother={()=>{setReport(null);setComparisonFile(null);}}/>:<><SectionCard className="global-verification-upload" title="Upload comparison image" description="The source is fingerprinted temporarily, searched globally, then removed from temporary processing storage."><ComparisonUploader file={comparisonFile} previewUrl={comparisonPreviewUrl} dimensions={comparisonDimensions} onFile={chooseFile} onRemove={()=>setComparisonFile(null)} onPreview={()=>setPreview({asset:{title:comparisonFile.name,mimeType:comparisonFile.type,...comparisonDimensions},sourceUrl:comparisonPreviewUrl})}/></SectionCard>{processing?<VerificationProgress/>:<div className="verification-run"><Button icon={Play} onClick={findMatches} disabled={!comparisonFile}>Analyze & find matches</Button><p>Exact SHA-256 matches rank first, followed by the closest 256-bit visual fingerprints.</p></div>}</>}<SectionCard className="verification-history" title="Verification history" description="Saved global match reports and the thresholds used for each search." action={<Button type="button" size="sm" variant="ghost" icon={RefreshCw} onClick={loadHistory}>Refresh</Button>}><VerificationHistory reports={history} loading={loadingHistory} onOpen={openHistory}/></SectionCard>{preview?<AssetPreviewModal asset={preview.asset} sourceUrl={preview.sourceUrl} onClose={()=>setPreview(null)}/>:null}</>;
}
